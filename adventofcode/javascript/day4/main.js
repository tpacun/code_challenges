...
let counterPartTwo = 0
let notCounted = []
pairsMinMax.forEach((pair) => {
     if (pair[0].max < pair[1].min || pair[1].max < pair[0].min) {
        counterPartTwo ++
     } else {
        notCounted.push(pair)
     }
})

console.log(notCounted)
console.log(`Part Two Solution: ${pairsMinMax.length - counterPartTwo}`)